...
model.fit(x=x_train_sqzed,y=y_train_sqzed,epochs=2,validation_data=(x_valid_sqzed,y_valid_sqzed),
          callbacks=[
            tensor_flow_call_back
])

result=model.evaluate(x_test_sqzed,y_test_sqzed)
print("Lineage Regression Base Linea Loss  for 2 epochs"+str(result))
...
input_rnn = tfk.layers.Input(shape=(None,1))
simple_rnn = tfk.layers.SimpleRNN(1)(input_rnn)

rnn_model = tfk.Model(inputs=[input_rnn],outputs=[simple_rnn])
rnn_model.compile(optimizer=tfk.optimizers.SGD(0.01),loss=[tfk.losses.MSE])

log_dir = r'./logs'
tensor_flow_call_back = tf.keras.callbacks.TensorBoard(log_dir, histogram_freq=1)

rnn_model.fit(x=x_train,y=y_train,epochs=2,validation_data=(x_valid,y_valid),
          callbacks=[
            tensor_flow_call_back
])

result=rnn_model.evaluate(x_test_sqzed,y_test_sqzed)
print("Basic RNN for 2 epochs"+str(result))
